    ...
    setCsvState((prevState) => ({
      ...
      data: {
        ...
        mapped: prevState.data.mapped.map((row, index) => ({
          ...row,
          [newValue]: prevState.data.parsed[index]?.[oldValue] ?? null, // Show `null` explicitly
        })),
      },
    }));
  }
    ...
    setCsvState((prevState) => ({
      ...
      data: {
        ...
        mapped: prevState.data.mapped.map((row) => {
          ...
          return { ...rest, [value]: row[value] ?? null }; // Keep `null` explicitly
        }),
      },
    }));
  }
  ...
  function getSanitizedData({ data }: { data: Record<string, unknown>[] }) {
    return data.map((row) =>
      Object.keys(row).reduce(
        (acc, key) => ({
          ...acc,
          [key]: row[key] === undefined ? "" : row[key], // Keep `null` values intact
        }),
        {}
      )
    );
